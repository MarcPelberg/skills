...
[`reset-agentic-context`](reset-agentic-context/SKILL.md) — **Marc-made.** Safely starts AI-assisted work over without pretending that deleting every hidden folder is harmless. It inventories active instructions, memories, skills, hooks, plugins, MCP servers, and the visible skill registry; moves eligible items into recoverable quarantine; journals rollback before each change; and protects source code, Git, credentials, sessions, safety policy, and application runtimes. Ask for the current repository only, name specific repositories, or request a **full reset** to include every repository, every registered worktree, and the current user's Codex and Claude global settings. Exact plugin removal still requires approval by plugin name, and the skill must report residual slash-command skills instead of calling an incomplete result clean.
